...
description: 初中物理（八年级、九年级）练习题生成与知识点学习系统。根据人教版教材按年级、章节、知识点生成选择题、判断题、填空题、计算题、实验探究题，每题配套知识点卡片。Use when user asks to generate middle school physics practice questions, experiment analysis, formula application, or requests knowledge point review for Chinese 中考 physics.
---
...
### 电路分析题
提供电路图的文字描述（串联/并联、开关位置、电表连接方式），考查电路识别、故障分析和电学计算能力。

### 作图题（Markdown 格式）
用文字描述力的示意图、光路图、电路图等的画法要求，给出作图步骤和要点。
